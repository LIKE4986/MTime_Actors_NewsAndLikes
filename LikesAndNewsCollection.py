...
def __likes_collections(actors_url):
    ...
    broswer = webdriver.Chrome()
    url_star = actors_url
    broswer.get(url_star)
    content = broswer.execute_script("return document.documentElement.outerHTML")
    time.sleep(1)
    broswer.close()
    name = re.findall('<h2 style="font-size:36px;">(.*?)</h2>', content)
    participants = re.findall('<p id="ratingCount" style="">(.*?)</p>', content)
    likes = re.findall('<p id="finalRating" style="">(.*?)</p></div>', content)
    dic = {}
    dic['name'] = name
    ...
    print(actors_likes)
    # actors_likes.to_csv('./document/likes_info.csv', index=False, sep='^')
    info = {
        'name': name,
        'likes': likes,
        'participants': participants
    }
    with open("./document/likes_info.json", "a+") as file:
...
def __news_collections(url_star, name):
    ...
    if str(no_exist) == "['很抱歉，你要访问的页面不存在']":
        ...
        if not pages:
            ...
            all_pages = pages[-1]
            print(all_pages)
            print("type allpages", type(all_pages))
            ...
            print(news_all)  # 该明星新闻框架
        print(len((news_all)))  # 该明星所有新闻页数（每页最多10则）
...
def __get_all():
    ...
    file_done:      记录done_url的txt文件
    urls:           演员链接
    name_content:   获取演员姓名的html内容
    actor_name:     演员姓名
    :return:
    """
    done_url = []
    with open("./document/alreadydone.txt", "r") as file_done:
        ...
        if each_url in done_url:
            ...
            try:
                ...
                try:
                    ...
                    with open("./document/alreadydone.txt", "a+") as file_done:
